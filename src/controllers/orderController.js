...
exports.createOrder = async (req, res) => {
  try {
    ...
    orderData.imageUrl = req.body.imageUrl;
...
exports.updateOrderStatus = async (req, res) => {
  try {
    ...
    if (status === "accepted" && !deliveryDate) {
      return res
        .status(400)
        .json({ error: "Delivery date is required when accepting an order" });
    }
...
exports.getPendingOrders = async (req, res) => {
  try {
    const orders = await Order.find({ status: "pending", is_active: true })
      .populate("items.furniture")
      .populate("items.woodType")
      .populate("user", "-password");
    res.json(orders);
...
exports.updateOrder = async (req, res) => {
  try {
    const { items, description, imageUrl } = req.body;
    ...
    if (!order) {
      return res.status(404).json({
        error:
          "Order not found or cannot be modified (only pending orders can be updated)",
      });
    }
